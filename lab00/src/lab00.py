...
def sigmoid(z: np.ndarray) -> np.ndarray:
    ...
    Requirement: the result must be finite for large-magnitude inputs.
        ...
        never `nan`.
    """
    # Preventing overflow by avoiding computation of exp(large positive number) and exp(large negative number)
    pos = z >= 0
    neg = ~pos
    out = np.empty_like(z, dtype=float)
    out[pos] = 1 / (1 + np.exp(-z[pos]))  # For positive z, compute exp(-z)
    out[neg] = np.exp(z[neg]) / (1 + np.exp(z[neg]))  # For negative z, compute exp(z)
    return out
# ============================ END TODO (Task 1) ==============================
...
def softmax_loop(z: list) -> list:
    ...
    """
    max_z = max(z)
    z_exp = [math.exp(x - max_z) for x in z]
    sum_z_exp = sum(z_exp)
    z = [x / sum_z_exp for x in z_exp]
    return z    
# ============================ END TODO (Task 2) ==============================
...
def entropy(p: np.ndarray) -> float:
    ...
    """
    mask = p > 0
    return -np.sum(p[mask] * np.log(p[mask] + 1e-12))
...
def focal_loss(p: np.ndarray, q: np.ndarray, gamma: float = 2.0,
    ...
    return -np.sum(p * alpha * (1 - q) ** gamma * np.log(q + 1e-12))
